...
const str_use_sms = "{"Enter the verification code sent to your phone"|translate|escape:javascript}";
...
const str_sms_wait_until = "{"Please wait %s seconds before sending an SMS again."|translate|escape:javascript}";
...
window.tfSmsResendDelay = {$TF_SMS_RESEND_DELAY|intval};
{/footer_script}
  ...
  <section id="tf-form" class="tf profile-section">
    ...
    <div id="tf_select_method">
      <div class="tf-select-method row-flex">
        ...
        {if $TF_STATUS_SMS}
        <div class="tf profile-section column-flex">
          <i class="tf-icon-method icon-comment"></i>
          <p>{"SMS"|translate|escape:html}</p>
          <p class="tf-desc-method">{"Use your phone to authenticate"|translate|escape:html}</p>
          <div class="save">
            <button class="btn btn-main row-flex tf-loading-mail" id="tf_validate_sms"><i id="tf_loading_sms" class="animate-spin icon-spin6"></i>{'Validate with SMS'|translate}</button>
          </div>
        </div>
        {/if}
      </div>
    ...
    <form class="tf-totp-form" id="tf_verify_code" method="post" action="{$F_ACTION}">
      ...
      <div id="tf_totp_sms" class="tf-send-again">
        <p>
            {'You didn’t get the SMS?'|translate}
            <span id="tf_send_again_sms" class="tf-underline">{'Send again.'|translate}</span>
            <span id="tf_send_again_in_sms">{'Send it again in %s seconds.'|translate}</span>
          </p>
      </div>
      <div class="save">
