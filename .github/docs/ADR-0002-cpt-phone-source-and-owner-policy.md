...
2. non-admin album owners must keep 2FA enabled while they own albums
...
6. Non-admin album owners are treated as requiring 2FA when CPT ownership detection reports at least one owned album.
7. Album owners without any configured 2FA method are redirected to Profile setup after login.
8. Album owners may disable an individual method only if at least one other 2FA method remains enabled.
9. When a non-admin user no longer owns albums, the plugin removes their stored 2FA methods on a later authenticated request.
...
- Automatic removal of stored 2FA methods when album ownership ends is policy-heavy and may need revisiting if requirements soften.
